...
fn main() {
    ...
    rt.block_on(async { chunk_into_days().await });
...
async fn process_station_year(station_file: String, year: usize) {
    ...
    let mut file = File::open(&station_file)
        .await
        .expect(format!("couldn't open station_file {station_file}").as_str());
    ...
    for day in 0..field_1.len() {
        ...
        field_1.next().unwrap().iter().for_each(|x| {
            day_vec.append(&mut x.to_le_bytes().to_vec());
        });
        field_2.next().unwrap().iter().for_each(|x| {
            day_vec.append(&mut x.to_le_bytes().to_vec());
        });
        field_3.next().unwrap().iter().for_each(|x| {
            day_vec.append(&mut x.to_le_bytes().to_vec());
        });
        ...
        println!(
            "{year}: {:?} saving {day_file}",
            std::thread::current().id()
        );
        tokio::fs::create_dir_all(station_dir)
            .await
            .expect("failed to create directory for station");
        tokio::fs::write(day_file, day_vec)
            .await
    ...
    assert!(
        field_1.next().is_none(),
        "Something went wrong, as there are still days of data to iterate through"
    );
    assert!(
        field_2.next().is_none(),
        "Something went wrong, as there are still days of data to iterate through"
    );
    assert!(
        field_3.next().is_none(),
        "Something went wrong, as there are still days of data to iterate through"
    );
}

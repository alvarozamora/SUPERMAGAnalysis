...
fn main() {
    ...
    let (projections_complete, auxiliary_complete) = read_dark_photon_projections_auxiliary().unwrap();
    println!(
...
fn read_dark_photon_projections_auxiliary() -> Result<(Arc<ProjectionsComplete>, Arc<<DarkPhoton as Theory>::AuxiliaryValue>), Box<dyn std::error::Error>> {
